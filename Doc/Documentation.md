...
EDK II stands for EFI Development Kit II.
Think of it this way: The 2000-page UEFI Specification PDF you downloaded is just a rulebook. It is a theoretical document that says, "A motherboard must do X, Y, and Z to boot an operating system." EDK II is the actual code that enforces those rules. It is a massive, open-source project (managed by a community called TianoCore, originally started by Intel).
When companies like Asus, Gigabyte, or Dell build a new motherboard, they don't write the BIOS from scratch. They download EDK II, add their custom logo and fan-control menus, and flash it onto the motherboard's ROM chip.
By using EDK II for VoidHash, you are using the exact same enterprise-grade development environment that hardware engineers use. Instead of writing 500 lines of raw C code to translate memory addresses just to print a single character to the screen, EDK II provides a library (UefiLib.h) that lets you just type Print(L"Hello");. It handles the hardware translation for you.
